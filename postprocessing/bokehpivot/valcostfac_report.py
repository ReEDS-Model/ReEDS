...
def build_html(output_dir, core_path):
    ...
    fig_lvoe2 = figure(output_dir, 'lvoe_vs_lcoe_byyear.png', n + 1,
                       'LVOE against LCOE for each new-build bin, by model year.',
                       'One dot per class-region-bin the model built that year, for the same model '
                       'years as the map figures. x is the model&rsquo;s lcoe output divided by the '
                       'mandate multiplier, so it is the unforced cost; y is new-build LVOE from '
...
{sec('07', 'LVOE against the LCOE of capacity built',
     ...
     'builds.</p></div>', fig_lvoe, lvoe_table, fig_lvoe2)}
