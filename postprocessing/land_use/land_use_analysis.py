...
def getLandUse(scen_path, jsonfile, rev_paths, reeds_path, tech, capacity_col="capacity_ac_mw", sc_base_path=None):
    ...
    if sc_base_path is not None:
        ...
        if isinstance(rev_paths.sc_file, str) and "Supply_Curve_Data" in rev_paths.sc_file:
            ...
            # Try with Supply_Curve_Data prefix first, then without
            candidate1 = os.path.join(sc_base_path, "Supply_Curve_Data", rel)
            candidate2 = os.path.join(sc_base_path, rel)
            rev_paths.sc_file = candidate1 if os.path.exists(candidate1) else candidate2
            print(f"...local sc_file resolved to: {rev_paths.sc_file}")
