"""
Interactive Bokeh dashboard for ReEDS-Proxy.
...
from surrogate_predict import DIMENSION_ENCODING, clip_physical_bounds, predict   # noqa: E402
from surrogate_artifact_cache import load_artifact_cached   # noqa: E402
...
def _artifact_for_layer(short: str, name: str):
    ...
    if name not in paths:
        ...
        return None
    try:
        art = load_artifact_cached(paths[name])
    except Exception:
        OOF_CACHE_BY_LAYER[cache_key] = None
        return None
...
def _get_artifact(name: str) -> dict | None:
    ...
    if needs_reload:
        art = load_artifact_cached(expected_path)
...
header = Div(text="""
<h2 style='margin:0'>ReEDS-Proxy — <span style='color:#0b6efd'>Stage 2</span> Interactive Panel</h2>
...
shows the ReEDS-style stacked portfolio (actual vs. ReEDS-Proxy
prediction). The bottom panel shows per-category proxy uncertainty
...
actual is left blank and only the ReEDS-Proxy prediction is shown.</p>
""")
...
def _format_metrics(
    ...
    if actual_row is None:
        if surrogate_ms is not None:
            lines.append(
                f"<b>ReEDS-Proxy runtime:</b> {surrogate_ms:.1f} ms "
        ...
        lines.append("<i>No training run matches this exact design — "
                     "showing the ReEDS-Proxy prediction only.</i>")
    ...
    if not np.isnan(runtime_actual) and surrogate_ms is not None and surrogate_ms > 0:
        ...
        lines.append(
            f"<b>Runtime:</b> ReEDS {runtime_actual/60:,.1f} min vs ReEDS-Proxy "
    ...
    if artifact is None:
        ...
        if actual_row is None:
            status_div.text = ("<b style='color:#a60'>This exact design isn't in the training "
                               "data — showing the ReEDS-Proxy prediction only.</b>")
        elif used_oof:
            status_div.text = (
...
for sel in design_selects.values():
    # Select widgets should react immediately. Sliders use value_throttled so
    # an expensive prediction runs only once, when the user releases the
    # handle (a direct track click also produces one final throttled value).
    callback_property = "value_throttled" if isinstance(sel, Slider) else "value"
    sel.on_change(callback_property, _on_change)
    ...
    if missing_cols and df is not None and not df.empty:
        ...
        if miss_figs:
            warn_header = Div(text=(
                ...
                f"ReEDS-Proxy has no answer for these.</div>"
            ))
            miss_rows = [
...
eval_tab = TabPanel(
    child=column(
        Div(text="<h2 style='margin:8px 0 4px 0'>ReEDS-Proxy "
                 "&mdash; Evaluation</h2>"
