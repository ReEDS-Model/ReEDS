...
# --- Compat shim: ngboost 0.5.8 calls sklearn.utils.validation.check_X_y
#     with the newer kwarg ``ensure_all_finite``, but sklearn 1.2.2 (pinned in
#     this env) only knows ``force_all_finite``. Wrap the two functions so the
#     kwarg is translated on the way in. Safe no-op on newer sklearn.
import sklearn.utils.validation as _skval
import sklearn.utils as _skutils
def _make_finite_shim(_orig):
    ...
    _wrapped._ensure_all_finite_shim = True
    return _wrapped
for _mod in (_skval, _skutils):
    for _name in ("check_X_y", "check_array"):
        _fn = getattr(_mod, _name, None)
        if _fn is not None and not getattr(_fn, "_ensure_all_finite_shim", False):
            setattr(_mod, _name, _make_finite_shim(_fn))
# Also patch inside ngboost, which does `from sklearn.utils import ...` at import time
try:
    import ngboost.ngboost as _ngb
    for _name in ("check_X_y", "check_array"):
        _fn = getattr(_ngb, _name, None)
        if _fn is not None and not getattr(_fn, "_ensure_all_finite_shim", False):
            setattr(_ngb, _name, _make_finite_shim(_fn))
except ImportError:
    pass
...
class Config:
    ...
    data_path: str = "../inputs/overall_ml_numeric_merged.csv"
    ...
    # STAGE 2: when both fields are supplied AND xgb is in the model list,
    # each XGB per-output fit gets its own monotone_constraints vector
    # (see surrogate_constraints.xgb_monotone_vector).
    x_cols: Optional[list] = None
    y_cols: Optional[list] = None
    # When True, disable the per-output monotone_constraints wrapper for
    # XGB and fall back to a plain XGBRegressor. Useful when the sample
    # size is too small for strict monotonicity (n=486) to be helpful.
    disable_monotone_xgb: bool = False


class MonotoneXGBMultiOutput:
    """MultiOutputRegressor-alike that fits one XGB per output with a
    per-output monotone_constraints vector.

    Used only when x_cols and y_cols are known at model construction time.
    Falls back to a plain MultiOutputRegressor(XGBRegressor(...)) otherwise.
    """

    def __init__(self, xgb_kwargs: dict, x_cols: list, y_cols: list):
    ...
    def fit(self, X, Y):
        if not HAS_XGBOOST:
            raise RuntimeError("XGBoost is required for MonotoneXGBMultiOutput")
        self.estimators_ = []
        Y = np.asarray(Y)
        if Y.ndim == 1:
            Y = Y.reshape(-1, 1)
        for i, ycol in enumerate(self.y_cols):
            mc = xgb_monotone_vector(ycol, self.x_cols)
            kw = dict(self.xgb_kwargs)
    ...
    def predict(self, X):
        parts = [e.predict(X).reshape(-1, 1) for e in self.estimators_]
...
def get_model(name: str, n_outputs: int, config: Config):
    ...
    if name == "knn":
        ...
        if HAS_XGBOOST:
            ...
            # STAGE 2: per-output monotone constraints when x_cols/y_cols known.
            if (
                not config.disable_monotone_xgb
                and config.x_cols is not None
                and config.y_cols is not None
                and n_outputs == len(config.y_cols)
            ):
                mono = MonotoneXGBMultiOutput(
                    xgb_kw, config.x_cols, config.y_cols
                )
                return mono, "XGBoost (Gradient Boosting, monotone)"
...
def load_data(config: Config):
    ...
    # STAGE 2: enforce non-negativity on training targets so the fitter
    # never sees an impossible negative in cap_* / tran_* / non-storage gen_*
    # / non-incentive cost_*.  Stage 1 clipped only at inference time.
    Y = clip_train_targets(Y, y_cols)
...
def run_pipeline(config: Config):
    ...
    # STAGE 2: expose x_cols / y_cols to get_model so the monotone XGB
    # wrapper can look up per-output rules.
    config.x_cols = list(x_cols)
    config.y_cols = list(y_cols)
...
def main():
    ...
    parser.add_argument(
        "--data", type=str,
        default=str(_STUDY_ROOT / "inputs" / "overall_ml_numeric_merged.csv"),
    ...
    parser.add_argument(
        "--no_monotone", action="store_true",
        help="Disable per-output monotone_constraints for XGB (falls back "
             "to plain XGBRegressor). Recommended at n=486 where strict "
             "monotonicity over-regularizes.",
    )
    ...
    config = Config(
        ...
        disable_monotone_xgb=args.no_monotone,
    )
