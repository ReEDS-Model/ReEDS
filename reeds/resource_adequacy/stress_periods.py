...
def get_stress_periods(case, sw, t, iteration):
    ...
    repperiods = pd.read_csv(
        Path(sw['casedir'], 'inputs_case', 'rep', 'set_szn.csv')
    ).rename(columns={'*szn':'actual_period'})
    stressperiods_this_iteration = pd.read_csv(
        os.path.join(
            sw['casedir'], 'inputs_case', f'stress{t}i{iteration}', 'period_szn.csv')
    )
    for df in [repperiods, stressperiods_this_iteration]:
        df['start'] = df.actual_period.map(reeds.timeseries.h2timestamp)
        df['end'] = (
            df['start']
            + (
    ...
    ## Also don't repeat representative periods. There is a risk here
    ...
    covered_periods = pd.concat([repperiods, stressperiods_this_iteration])
    covered_hours = [
        pd.date_range(row.start, row.end, freq='1h')
        for i,row in covered_periods.iterrows()
    ]
