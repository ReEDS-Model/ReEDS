...
In that case successive amounts of new point-of-interconnection (POI) capacity in each zone are priced in increasing cost bins (built at run time from `inputs/transmission/raw_interconnection_TSC_data.csv`), with an unlimited backstop bin priced at `GSw_POIUpperCost` so that reinforcement above the binned capacities remains feasible at a high cost.
...
With `numpoibins` = 1 (the default flat adder) the wind, solar, CSP, and geothermal supply-curve reinforcement cost is retained.
