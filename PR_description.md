...
- **Hardening & hygiene**: the USD2024→2004$ dollar-year conversion via `get_inflatable()`/`dollaryear.csv`; replacing silent-default `sw.get()` switch lookups with direct indexing; relocating `make_poi_supply_curve.py` into `reeds/input_processing/`; renaming the mislabeled `capacity_MW`→`capacity_GW` column; consolidating and restyling the documentation; and diff-hygiene cleanup.
